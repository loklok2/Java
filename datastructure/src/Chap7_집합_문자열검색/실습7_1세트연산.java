...
class Sets {
	...
	public void SimpleUnion(int i, int j){ // Replace the disjoint sets with roots i and j, i != j with their union
		// i,j는 임의 노드로서 각 집합 i의 root를 찾아 j의 root에 연결
		
	}

	public int SimpleFind(int i) { // Find the root of the tree containing element i
		while (parent[i] > 0)
			i = parent[i];
		return i;
	}
